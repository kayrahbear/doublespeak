...
positive_data_set = open("training_data/positive.txt", "r").read()
negative_data_set = open("training_data/negative.txt", "r").read()

all_words = []
data_documents = []

#  j is adjective, N is a noun, and v is verb
allowed_word_types = ["J", "N", "V"]

for p in positive_data_set.split('\n'):
    data_documents.append((p, "pos"))
...
for p in negative_data_set.split('\n'):
    data_documents.append((p, "neg"))
    words = word_tokenize(p)
    pos = nltk.pos_tag(words)
    for w in pos:
        if w[1][0] in allowed_word_types:
...
pickle.dump(data_documents, save_documents)
...
featuresets = [(find_features(rev), category) for (rev, category) in data_documents]
...
save_classifier = open("pickled_algorithms/originalnaivebayes5k.pickle", "wb")
